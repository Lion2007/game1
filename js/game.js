...
Game.prototype.init = function() {
	...
	this.start_message = "Доцю, сину, Натисни пробіл для старту"
...
Game.prototype.handle_collision = function(other_object, relative_velocity, relative_rotation) {
	...
	if (vel > 32) {
		this.message = "Дитино, ти можеш краще";
	...
	if (this.landed) {
		...
		if (current_time - this.landing_time > 2000 && !this.assigned_score) {
			...
			if (angle_diff*180/Math.PI > 20) {
				this.message = "Дитино, човник перевернувся!";
				this.game_status = "GAME OVER";
				this.landing_type = 2;
				this.lander.flames_on = true;
			}
			else if (this.landing_vel > 16 && this.landing_vel < 32) {
				this.message = "Трохи зажорстко, але уже набагато краще!!!";
				this.game_status = "GAME OVER";
				this.score += 50;
				this.landing_type = 1;
				...
				this.message = "Дитино, ти Молодчинка!!!";
				this.score += 100;
				this.landing_type = 0;
